...
const Header = ({ activeView, onSetView, onCreateNew }) => {
  ...
  return (
    <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
      <div>
        <h1 className="text-3xl font-bold text-violet-700">AIT Australia QA Dashboard</h1>
        <p className="text-gray-500 mt-1">Comprehensive checklists and historical reports.</p>
      </div>
      <div className="flex items-center space-x-2 mt-4 sm:mt-0">
        <button 
          onClick={onCreateNew}
          className={`flex items-center px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${isChecklistActive ? 'bg-violet-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-violet-50'}`}
        >
          <CheckSquare className="w-5 h-5 mr-2" /> Checklist
        </button>
        <button 
          onClick={() => onSetView('reports')}
          className={`flex items-center px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${isReportsActive ? 'bg-violet-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-violet-50'}`}
        >
...
const SubmissionProgress = ({ checklist, onSubmit }) => {
    ...
    return (
         <div className="bg-white p-4 rounded-xl shadow-md border border-gray-200 mt-8 flex justify-between items-center">
            <div className="flex-grow mr-6">
                <h3 className="font-semibold text-gray-700">Overall Progress ({progress}%)</h3>
            ...
            <button 
                onClick={onSubmit} 
                className="flex items-center px-5 py-3 bg-green-500 text-white font-bold rounded-lg shadow-md hover:bg-green-600 transition-all whitespace-nowrap"
            >
...
const ReportView = ({ report, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={onBack}
        className="flex items-center text-sm font-semibold text-gray-600 hover:text-violet-700 transition-colors mb-2"
      >
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between items-start">
                <div className='mb-4 sm:mb-0'>
                    <h2 className="text-3xl font-bold text-violet-700">{details.name}</h2>
                    <a href={details.url} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-violet-600 transition-colors break-all">{details.url}</a>
                </div>
                <div className="text-left sm:text-right text-sm text-gray-600 flex-shrink-0 sm:ml-4">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
                <h3 className="font-semibold text-gray-700">Final Score: {completion}% Complete</h3>
                <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                    <div className="bg-violet-600 h-2.5 rounded-full" style={{ width: `${completion}%` }}></div>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <SummaryStat icon={Clock} value={stats.na} label="Pending (N/A)" color="text-gray-600" />
...
const InputField = ({ label, icon: Icon, isReadOnly, ...props }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input type="text" {...props} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-violet-500 focus:border-violet-500 disabled:bg-gray-100" disabled={isReadOnly}/>
    </div>
  </div>
);
...
const SelectField = ({ label, icon: Icon, value, onChange, options, isReadOnly }) => (
   <div>
    ...
    <div className="relative">
      <Icon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
      <select value={value} onChange={onChange} disabled={isReadOnly} className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-violet-500 focus:border-violet-500 appearance-none disabled:bg-gray-100">
      ...
      {Object.entries(checklistData).map(([phase, { icon: Icon, subtitle }]) => (
        <li key={phase}>
          <button 
            ...
            className={`w-full text-left flex items-start p-3 rounded-lg transition-colors ${activePhase === phase ? 'bg-violet-100 text-violet-700' : 'hover:bg-gray-100'}`}
          >
            <Icon className={`w-6 h-6 mr-3 mt-0.5 flex-shrink-0 ${activePhase === phase ? 'text-violet-600' : 'text-gray-500'}`} />
            <div>
    ...
    <div className="space-y-4">
      {items.map((item, index) => {
        const statusColorClasses = {
            ...
            'N/A': 'border-gray-200 bg-white'
        };
        return (
          <div key={index} className={`p-4 rounded-xl shadow-sm border ${statusColorClasses[item.status]}`}>
            ...
            <textarea
              ...
              readOnly={isReadOnly}
              className="w-full p-2 border border-gray-300 rounded-lg text-sm mb-2 focus:ring-1 focus:ring-violet-500 focus:border-violet-500 read-only:bg-gray-100"
            />
            <div className="flex items-center space-x-4">
              <select value={item.priority} onChange={e => onChange(phase, index, 'priority', e.target.value)} disabled={isReadOnly} className="py-1 px-2 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-violet-500 disabled:bg-gray-100">
                {priorities.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
              <select value={item.status} onChange={e => onChange(phase, index, 'status', e.target.value)} disabled={isReadOnly} className="py-1 px-2 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-violet-500 disabled:bg-gray-100">
  ...
  if (reports.length === 0) {
    return (
      <div className="text-center bg-white p-10 rounded-xl shadow-sm border border-gray-200">
        ...
        <p className="text-gray-500 mt-2">Get started by creating your first QA report.</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="border-b-2 border-gray-200">
            <tr>
              <th className="p-3 text-sm font-semibold text-gray-600">Project Name</th>
              <th className="p-3 text-sm font-semibold text-gray-600">Assigned To</th>
            ...
            {reports.map(report => {
              ...
              return (
                <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-3 text-gray-600">{report.details.assignedTo}</td>
                  <td className="p-3 text-gray-600">{new Date(report.submittedAt).toLocaleDateString()}</td>
                  <td className="p-3">
